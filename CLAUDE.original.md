...
- All env validated through `@vanta-base-admin/env` — apps must not read `process.env` directly
...
- UI components: always use shadcn UI components from `@vanta-base-admin/ui` — never raw HTML inputs, selects, buttons, or dialogs when a shadcn equivalent exists or can be added