...
describe("UsersService", () => {
  ...
  describe("create", () => {
    it("resolves role slug from roleId and delegates to Better Auth admin API", async () => {
      ...
      vi.mocked(auth.api.adminCreateUser).mockResolvedValue(expectedResult as never);
  ...
  describe("assignRole", () => {
      ...
      vi.spyOn(db, "update").mockReturnValueOnce({
        set: () => ({
          where: () => ({
            returning: vi
              .fn()
              .mockResolvedValue([{ ...mockUserRow, role: "admin", roleId: "role_admin" }]),
          }),
        }),
      } as never);
    ...
    it("throws NotFoundException when role not found", async () => {
      ...
      await expect(service.assignRole("u1", "role_missing", "actor-id")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("ban", () => {
    ...
    it("throws NotFoundException when user not found", async () => {
      ...
      await expect(service.ban("missing", { banReason: "x" }, "actor-id")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("softDelete", () => {
    it("throws ForbiddenException when deleting own account", async () => {
      await expect(service.softDelete("u1", "u1", "u1")).rejects.toThrow(ForbiddenException);
    });
