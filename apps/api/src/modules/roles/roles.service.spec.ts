...
describe("RolesService", () => {
  ...
  describe("update", () => {
    it("throws NotFoundException when role not found", async () => {
      ...
      await expect(service.update("missing", { name: "X" }, "actor-id")).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe("remove", () => {
    ...
    it("throws BadRequestException when deleting system role", async () => {
      ...
      await expect(service.remove("role_user", "actor-id")).rejects.toThrow(BadRequestException);
    });

    it("allows deleting non-system roles", async () => {
      mockDbQueryRolesFindFirst.mockResolvedValue(mockRoleMod);
      mockDbSelect.mockReturnValue([]);
      await expect(service.remove("role_mod", "actor-id")).resolves.not.toThrow();
    });
  });
  ...
  describe("create", () => {
    it("throws BadRequestException when slug already exists", async () => {
      mockDbQueryRolesFindFirst.mockResolvedValue(mockRoleMod);
      await expect(service.create({ name: "Moderator" }, "actor-id")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
