...
describe("InventoryController", () => {
  ...
  describe("getProduct", () => {
    it("propagates NotFoundException when product not found", async () => {
      mockService.getProduct.mockRejectedValue(new NotFoundException("Product not found"));
      await expect(controller.getProduct(mockUser, "p1")).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe("createMovement", () => {
    it("calls service.createMovement and returns movement", async () => {
      ...
      const result = await controller.createMovement(mockReq, mockUser, "p1", dto as any);
    ...
    it("propagates UnprocessableEntityException for invalid movement", async () => {
      mockService.createMovement.mockRejectedValue(
        new UnprocessableEntityException("Movement would result in negative stock quantity"),
      );
      await expect(
