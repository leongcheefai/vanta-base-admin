...
export class InventoryService {
  ...
  async createCategory(userId: string, dto: CreateCategoryDto, ctx?: AuditContext) {
  ...
  async updateCategory(userId: string, id: string, dto: UpdateCategoryDto, ctx?: AuditContext) {
    const before = await db.query.inventoryCategory.findFirst({
      where: and(eq(schema.inventoryCategory.id, id), eq(schema.inventoryCategory.userId, userId)),
    });
  ...
  async deleteCategory(userId: string, id: string, ctx?: AuditContext) {
    ...
    const [result] = await db
      ...
      .where(
        and(eq(schema.inventoryProduct.categoryId, id), isNull(schema.inventoryProduct.deletedAt)),
      );
    if ((result?.total ?? 0) > 0) {
  ...
  async createProduct(userId: string, dto: CreateProductDto, ctx?: AuditContext) {
    try {
  ...
  async updateProduct(userId: string, id: string, dto: UpdateProductDto, ctx?: AuditContext) {
    ...
    try {
      const [product] = await db
        ...
        .set(updates)
        .where(and(eq(schema.inventoryProduct.id, id), eq(schema.inventoryProduct.userId, userId)))
        .returning();
      if (!product) throw new NotFoundException("Product not found");
  ...
  async createMovement(
    ...
    if (newQuantity < 0) {
      throw new UnprocessableEntityException("Movement would result in negative stock quantity");
    }
