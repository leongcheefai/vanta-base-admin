...
export class AuditService {
  async record(input: AuditRecordInput, ctx?: AuditContext, tx?: TxOrDb): Promise<void> {
    const client = tx ?? db;
  ...
  async list(params: {
    ...
    if (params.targetType) conditions.push(eq(schema.auditLog.targetType, params.targetType));
    ...
    const [data, totals] = await Promise.all([
      db
        ...
        .leftJoin(
          targetUser,
          and(eq(schema.auditLog.targetId, targetUser.id), eq(schema.auditLog.targetType, "user")),
        )
        .leftJoin(
          schema.roles,
          and(
