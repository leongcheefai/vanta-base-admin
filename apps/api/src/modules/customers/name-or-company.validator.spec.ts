...
import {
	HasNameOrCompany,
	HasNameOrCompanyConstraint,
} from "./name-or-company.validator";
...
describe("HasNameOrCompanyConstraint", () => {
	...
	it("accepts object with firstName", () => {
		expect(
			constraint.validate(undefined, makeArgs({ firstName: "Jane" })),
		).toBe(true);
	});

	it("accepts object with lastName", () => {
		expect(
			constraint.validate(undefined, makeArgs({ lastName: "Smith" })),
		).toBe(true);
	});

	it("accepts object with company only (company-only shape)", () => {
		expect(
			constraint.validate(undefined, makeArgs({ company: "Acme Corp" })),
		).toBe(true);
	});
...
describe("HasNameOrCompany decorator integration", () => {
	it("passes when firstName is provided (create shape)", async () => {
		...
		expect(errors.filter((e) => e.property === "_nameOrCompany")).toHaveLength(
			0,
		);
	});
	...
	it("fails when no name fields provided (empty create shape)", async () => {
		...
		const errors = await validate(dto);
		expect(
			errors.filter((e) => e.property === "_nameOrCompany").length,
		).toBeGreaterThan(0);
	});
});
